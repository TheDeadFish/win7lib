...
// 
#define STANDALONE
#define GetModuleHandleA getModuleEx

int cmpi(char ch1, char ch2) {
    return toupper(ch1) == toupper(ch2); }
int strSicmp(const char* str1, const char* str2) {
    for(;; str1++, str2++) { if(!*str2) return 1;
        if(!cmpi(*str1, *str2)) return 0; } }
HMODULE getModuleEx(LPCSTR name) {
    LPCSTR newName = name;
    if(strSicmp(name, "kernel32")) newName = "krnl32ex.dll";
    if(strSicmp(name, "ntdll")) newName = "ntdllex.dll";
    HMODULE hMod = LoadLibraryA(newName);
    if(!hMod) hMod = LoadLibraryA(name);
    return hMod;
}
